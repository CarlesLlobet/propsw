...
public class FordFulkerson<T> {	
	...
	//Mètode que retorna un String amb una representació de la matriu 
	...
	private void computaGrafResidual(ArrayList<Integer> camino, Graf<T> graf) throws IOException {
		...
		//Calculamos el máximo flujo que puede pasar por el camino.
	...
	//Método a sobreescribir por las clases que heredan de FordFoulkerson
	...
	//Esta implementación no tiene en cuenta los costes.
	public ArrayList<Integer> dameCamino(Graf<T> graf) throws IOException {		
		...
		if(s == t){
			//S'ha trobat el sumidero. Começa el backtracking.
		...
		while (i < outs.size() && !found) {
			...
			if(graf.getCapacidadAresta(idAresta)>0){
				//El node no está saturat (Capacitat de la aresta > 0)
				initBUp = s;
				s = outs.get(i);
				if(!visited[s]){
					visited[s]=true;
				...
				s = initBUp;
				if(helper != null){
					//Es fa backtracking només si s'ha trobat el sumidero. Es van afegint les variables
		...
		//Si no s'ha trobat el sumidero la variable toReturn serà null
		return toReturn;
	}
